...
def generate_taskset_mode_dependent_v2(num_tasks, utilization, hi_prob=0.5, hi_factor=2.0, tight_factor=2.0):
    ...
    for i in range(num_tasks):
        ...
        if is_hi:
            ...
            # LO-Task: Urgent!
            # [TWEAK 2] Make LO-tasks extremely urgent to force AMC failure.
            # D between C and 0.5*T (Competing directly with HI-tasks D=0.4T)
            # In AMC: HI-tasks (D=0.4T) pre-empt LO-tasks (D=0.5T) -> LO tasks die.
            # In VP-AMC: LO-tasks (D=0.5T) pre-empt HI-tasks (D=T in LO-mode) -> Everyone lives.
            upper_bound = max(c_lo, period * 0.5)
...
def main():
    # [TWEAK 3] Parameters optimized for maximizing the gap
    num_tasks = 20
    hi_prob = 0.6       # Increased HI tasks (more interference for AMC)
    hi_factor = 2.0 
    tight_factor = 2.5  # HI-mode deadline is 0.4T (Forces High Priority in AMC)
    ...
    all_data = {
        'parameters': {
            ...
            'note': "V2.2: Aggressive Setup (LO D=0.5T vs HI D=0.4T)"
        },
        'util_points': util_points,
        'tasksets': {} 
    }
    
    print(f"Generating Tasksets V2.2 (Max Gap Scenario)...")
