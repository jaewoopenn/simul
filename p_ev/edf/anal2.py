...
# ---------------------------------------------------------
# 2. 알고리즘 로직 (sLLF는 기존 유지, NEW_ALGO 수정)
# ---------------------------------------------------------
...
def calculate_new_algo_power(current_time, active_evs, grid_capacity, max_ev_power, time_step) -> Tuple[List[float], List[str]]:
    ...
    status_codes = [""] * count  # 초기화 (나중에 채움)
    remaining_grid = grid_capacity
    
    # ---------------------------------------------------------
    # Step 0: 상태 변수 및 Ratio 계산
    # ---------------------------------------------------------
    ev_status = []
    ev_ratios = [] # 각 EV의 Ratio 저장용 리스트
    ...
    for i, ev in enumerate(active_evs):
        ...
        if time_to_deadline > EPSILON:
            ...
            ratio = 1.0 if ev.remaining > EPSILON else 0.0
    ...
    for st in ev_status:
        ...
        if needed > EPSILON:
            ...
            status_codes[idx] = "MAND" # 접미사는 나중에 일괄 적용하거나 여기서 적용

    # ---------------------------------------------------------
    # Step 2: Minimax Optimization
    # ---------------------------------------------------------
    residual_needs = []
    residual_caps = []
    durations = []
    ...
    for i in range(count):
        ev = active_evs[i]
        rem_energy = ev.remaining - (allocation[i] * time_step)
        dur = max(time_step, float(ev.deadline) - current_time)
        durations.append(dur)
        residual_needs.append(rem_energy)
        
        phys_limit = min(max_ev_power, ev.remaining / time_step)
        cap = max(0.0, phys_limit - allocation[i])
        residual_caps.append(cap)
        
        if rem_energy > EPSILON: has_needs = True
    ...
    if remaining_grid > EPSILON:
        ...
        for i in range(count):
            ...
            if room > EPSILON:
                l = (float(active_evs[i].deadline) - current_time) - (active_evs[i].remaining / max_ev_power)
                urgency.append((l, i))
    ...
    # ---------------------------------------------------------
    # Step 4: Finalize Status Strings with Ratios
    # ---------------------------------------------------------
    final_status = []
    for i in range(count):
        code = status_codes[i]
        ratio_str = f"({ev_ratios[i]:.2f})"
        
        if code == "":
            final_status.append(f"WAIT{ratio_str}")
        else:
            final_status.append(f"{code}{ratio_str}")
            
    return allocation, final_status
...
def run_simulation(ev_set: List[EVRequest], algorithm: str, enable_log: bool = False) -> bool:
    ...
    while finished_cnt < total_evs:
        ...
        if ready_queue:
            ...
            # --- 상세 로그 출력 ---
            if enable_log:
                print(f"-"*80) # 구분선 길이 조정
                # Status 칸 너비를 15로 늘림 (WAIT(0.xx) 등의 길이 대응)
                print(f"Time {current_time:5.1f} | Active EVs: {len(ready_queue)}")
                print(f"  {'EV_ID':<6} | {'Rem':<7} | {'DL':<4} | {'Mand':<5} | {'Alloc':<6} | {'Status':<15}")
                ...
                for i, ev in enumerate(ready_queue):
                    ...
                    print(f"  EV{ev.ev_id:<4d} | {ev.remaining:6.2f}  | {ev.deadline:<4d} | {m:5.2f} | {p:6.2f} | {st:<15}")
