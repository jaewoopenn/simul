...
def calculate_sllf_power(current_time, active_evs, grid_capacity, max_ev_power, time_step):
    ...
    current_laxities = []
    phys_limits = []
    min_req_allocations = []  # Peak-Shaving을 위한 최소 요구 전력 리스트

    EPSILON = 1e-6

    for ev in active_evs:
        # 1. Laxity 계산 (기존 로직 유지)
        ...
        # 2. 물리적 최대 한계 (배터리/충전기 한계)
        p_phys_limit = min(max_ev_power, ev.remaining / time_step)
        phys_limits.append(p_phys_limit)

        # 3. [수정됨] Peak-Shaving을 위한 '필요 최소 전력' 계산
        # 남은 시간 동안 균등하게 나누어 충전할 때 필요한 전력 (Uniform Rate)
        # remaining_time이 0 이하일 경우 max_ev_power로 처리하여 오류 방지
        valid_remaining_time = max(time_step, remaining_time)
        p_req = ev.remaining / valid_remaining_time
        
        # 물리적 한계를 넘을 순 없으므로 clamp 적용
        p_req = max(0.0, min(p_req, p_phys_limit))
        min_req_allocations.append(p_req)

    # ---------------------------------------------------------
    # [핵심 변경 사항] Work-Conserving -> Peak-Shaving
    # 기존: 물리적 한계(최대치)의 합이 용량 이내면 무조건 최대치 할당 (Greedy)
    # 변경: '필요 최소 전력'의 합이 용량 이내면, 딱 필요한 만큼만 할당 (Load Flattening)
    # ---------------------------------------------------------
    if sum(min_req_allocations) <= grid_capacity + EPSILON:
        return min_req_allocations

    # ---------------------------------------------------------
    # 그리드 용량이 부족한 경우 (Overload)
    # 기존 sLLF 로직(이진 탐색)을 사용하여 Laxity 기반으로 공정하게 전력 배분
    # ---------------------------------------------------------
    def get_total_power_for_target_L(target_L):
        total_p = 0.0
        allocs = []
        for i in range(count):
            # sLLF 공식: 목표 Laxity를 맞추기 위한 전력 계산
            req_p = (max_ev_power / time_step) * (target_L - current_laxities[i] + time_step)
            # 계산된 전력은 물리적 한계를 넘을 수 없음
    ...
    # 이진 탐색으로 그리드 용량에 가장 근접한(꽉 채우는) Target Laxity 탐색
    for _ in range(20):
        mid_L = (low_L + high_L) / 2.0
        p_sum, p_allocs = get_total_power_for_target_L(mid_L)
        if p_sum > grid_capacity:
            # 전력이 너무 많이 필요함 -> Target Laxity를 높여서(여유를 줘서) 전력을 줄임
            low_L = mid_L  
        else:
            # 전력이 남음 -> Target Laxity를 낮춰서(급하게) 전력을 늘림
            high_L = mid_L
            best_allocations = p_allocs

    # 부동소수점 오차 등으로 용량을 살짝 넘는 경우 스케일링
...
# ---------------------------------------------------------
# [New] 병렬 처리를 위한 작업자 함수
# ---------------------------------------------------------
def worker_task(congestion_level):
    """
    하나의 실험(Trial)을 수행하는 함수입니다.
    데이터를 생성하고 4가지 알고리즘을 모두 돌린 후 결과를 반환합니다.
    """
    # 병렬 프로세스마다 다른 랜덤 시드를 갖도록 초기화 (OS 시간에 기반)
    random.seed() 
    
    ev_requests = generate_ev_set(congestion_level)
    
    # 데이터가 없으면 모두 성공(1) 혹은 무시 처리, 여기선 편의상 Skip 로직을 위해 빈 결과 반환
    if not ev_requests:
        return congestion_level, None

    result_vector = {}
    
    # 4가지 알고리즘 실행
    for algo in ['EDF', 'FCFS', 'NEW_ALGO', 'sLLF']:
        success = run_simulation(ev_requests, algo)
        result_vector[algo] = 1 if success else 0
        
    return congestion_level, result_vector

# ---------------------------------------------------------
# 6. 메인 실행 (병렬화 적용)
# ---------------------------------------------------------
if __name__ == '__main__':
    # Windows 환경에서의 multiprocessing 오류 방지를 위해 if __name__ == '__main__': 필수
    
    start_time = time.time()
    congestion_levels = list(range(STRESS_START, STRESS_START+STRESS_NUM))
    
    # 전체 작업 목록 생성 (레벨당 TRIAL_NUM번 수행)
    # [(Level 5), (Level 5)..., (Level 6)...] 형태의 작업 리스트
    tasks = []
    ...
    print(f"Total Tasks: {len(tasks)} (Levels: {len(congestion_levels)} * Trials: {TRIAL_NUM})")

    # 병렬 처리 시작
    results_map = {level: {k: 0 for k in ['FCFS','EDF',  'NEW_ALGO', 'sLLF']} for level in congestion_levels}
    trial_counts = {level: 0 for level in congestion_levels}

    with Pool(processes=cpu_count()) as pool:
        # map을 통해 작업 분배 및 실행
        # worker_task가 (level, {algo_results}) 튜플을 반환함
        raw_results = pool.map(worker_task, tasks)
        
    # 결과 집계 (Aggregation)
    for level, res in raw_results:
        if res is None: continue # 데이터 생성 실패 등
    ...
    # 결과 정리 (비율 계산)
    ratios = {k: [] for k in ['FCFS','EDF',  'NEW_ALGO', 'sLLF']}
    print(f"\nComparison: sLLF vs NEW_ALGO")
    
    for level in congestion_levels:
        count = trial_counts[level]
        if count == 0: count = 1 # avoid zero div
        
        edf_rate = results_map[level]['EDF'] / count
        llf_rate = results_map[level]['FCFS'] / count
        sllf_rate = results_map[level]['sLLF'] / count
        new_rate = results_map[level]['NEW_ALGO'] / count
        
        ratios['EDF'].append(edf_rate)
        ratios['FCFS'].append(llf_rate)
        ratios['sLLF'].append(sllf_rate)
        ratios['NEW_ALGO'].append(new_rate)
        
        print(f"Level {level:2d}: EDF={edf_rate:.2f}, FCFS={llf_rate:.2f}, "
              f"sLLF={sllf_rate:.2f}, NEW={new_rate:.2f}")
    ...
    plt.plot(congestion_levels, ratios['EDF'], marker='o', label='Fluid-EDF', linestyle=':', color='gray', alpha=0.5)
    plt.plot(congestion_levels, ratios['FCFS'], marker='s', label='FCFS', linestyle='--', color='blue', alpha=0.5)
    ...
    plt.title('Performance Comparison (Parallelized)', fontsize=14)
